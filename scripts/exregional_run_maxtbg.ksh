    ...
    while [[ $CYCLE -ge $CYCLE_STOP ]]; do
        ...
        if [[ $run = "rtma3d" ]] ; then
            if [[ $dname == "hrrr" ]] ; then
                ...
                gesfile=${RRFS_GRIBDIR}/rrfs.${YYYYMMDD}/${HH}/rrfs.t${HH}z.prslev.f000.conus_3km.grib2
                if [[ ! -s $opsfile ]] ; then
...
cp $FIX_MINMAXT/aktz.bin .
cp $FIX_MINMAXT/conusexttz.bin .
cp $FIX_MINMAXT/conustz.bin .
cp $FIX_MINMAXT/conustz_ndfdonly.bin .

export pgm=rrfs_maxt.exe
#startmsg
${EXECDIR}/rrfs_maxt.exe > stdout 2>&1 
export err=$?
#cat $pgmout
