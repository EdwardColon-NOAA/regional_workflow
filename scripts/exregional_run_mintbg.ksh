    ...
    while [[ $CYCLE -ge $CYCLE_STOP ]]; do
        ...
        if [[ $run = "rtma3d" ]] ; then
            if [[ $dname == "hrrr" ]] ; then
                ...
                gesfile=${COMOUT_BASEDIR_ges}/rrfs.${YYYYMMDD}/${HH}/rrfs.t${HH}z.prslev.f012.conus_3km.grib2
                if [[ ! -e $opsfile ]] ; then
	          opsfile=$gesfile
	        fi
	    fi
...
cp $FIX_MINMAX/aktz.bin .
cp $FIX_MINMAX/conusexttz.bin .
cp $FIX_MINMAX/conustz.bin .
cp $FIX_MINMAX/conustz_ndfdonly.bin .

export pgm=rtma3d_mintgb
#startmsg
${EXECDIR}/rtma3d_mintbg  > stdout 2>&1 
export err=$?
#cat $pgmout
